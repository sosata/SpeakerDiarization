...
public class SignalProcessing {
    ...
    private double[] noise_levels_squared;
    ...
    public SignalProcessing(int length) {
        ...
        //creating Gaussian noise spectrum for later use
        noise_levels_squared = new double[FRAME_LENGTH];
        double[] noise = new double[FRAME_LENGTH];
        Random rnd = new Random();
        rnd.setSeed(0);
        double noise_max = 0;
        for (int i=0; i<FRAME_LENGTH; i++) {
            noise[i] = rnd.nextGaussian();
            if (Math.abs(noise[i])>noise_max)
                noise_max = Math.abs(noise[i]);
        }
        for (int i=0; i<FRAME_LENGTH; i++)
            noise[i] /= noise_max;
        Complex[] noise_fft = fft.transform(noise, TransformType.FORWARD);
        noise_levels_squared = this.spectral_power(noise_fft);
    ...
    double[] computeAutoCorrelationPeaks2(double[] power) {
        ...
        return findPeaks(ac_normalized);
